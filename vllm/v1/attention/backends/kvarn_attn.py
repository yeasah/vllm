...
class KVarNAttentionBackend(AttentionBackend):
    ...
    def customize_spec(cls, spec: AttentionSpec) -> AttentionSpec:
        """KVarN packs K+V for one group-tile into a single per-head record.

        Scales are tile-shared, so the natural unit is a whole block rather than
        a token slot. `state_content_bytes` is that record divided by the group,
        which reproduces exactly the page size `get_kv_cache_shape` below
        describes -- the identity the original docstring spells out. Upstream
        expressed this with a `TQFullAttentionSpec` / `TQSlidingWindowSpec`
        subclass carrying `tq_slot_size`; both were removed in favour of this
        hook, so the spec subclasses the PR added are no longer needed.
        """
        ...
        assert cfg.tile_bytes_aligned % cfg.group == 0, (
            f"KVarN tile ({cfg.tile_bytes_aligned}) must divide evenly by the "
            f"group ({cfg.group}) to express a per-slot content size."
        )
        return replace(
            spec, state_content_bytes=cfg.tile_bytes_aligned // cfg.group
        )
    ...
    def supported_kv_cache_layouts(cls) -> tuple[KVCacheLayout, ...]:
        ...
        Each (block, head) record is one contiguous run, which is the same byte
        ordering as the `(num_blocks, num_kv_heads, tile_bytes_aligned)` shape
        the kernels index. The N/C split within a record is nominal -- KVarN's
        scales are tile-shared, so only its own kernels address inside one.
        """
